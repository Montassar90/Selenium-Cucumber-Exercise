...
public class ProductsPageObject {
	...
	// Cast the driver to JavascriptExecutor and execute a script to scroll the window to the bottom of the page
	public void scrollDownPage() {
		((JavascriptExecutor) Setup.getDriver()).executeScript("window.scrollTo(0, document.body.scrollHeight);");
	}

	// Simulate a click on the scroll up button (assumes scrollUpBtn is defined elsewhere in the class)
	public void clickScrollUpBtn() {
		scrollUpBtn.click();
	}

	// Return the WebElement representing the products link (assumes productsLink is defined elsewhere in the class)
